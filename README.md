...
7. **Model validation, diagnostics, and prediction**  
   All created models are compared using BIC, adjusted R² and RMSE to balance fit, complexity, and predictive accuracy. Residual behavior is then assessed (normality, homoscedasticity, autocorrelation) and influential observations are identified via leverage, studentized residuals, and Cook’s distance. Lastly, the selected model is used to generate estimates of newborn weight under specified clinical scenarios, supported by clear 2D/3D visualizations.
